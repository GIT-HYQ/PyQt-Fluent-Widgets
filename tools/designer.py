...
def get_designer_path():
    ...
    dll_path = Path(
        f"{site_packages}/qt5_applications/Qt/plugins/designer/{dll_name}")
    if dll_path.exists():
        return str(path)

    plugin_dll_path = Path(
        f"{site_packages}/pyqt5_plugins/Qt/plugins/designer/{dll_name}")
...
print(str(project_dir / 'plugins'), get_designer_path())
