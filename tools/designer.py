...
def get_designer_path():
    ...
    if not path.exists():
        raise Exception(
            "Can't find available QtDesigner for current environment. You can try `pip install pyqt6-tools` to solve this problem.")
    ...
    dll_path = Path(
        f"{site_packages}/qt6_applications/Qt/plugins/designer/{dll_name}")
    if dll_path.exists():
        return str(path)

    plugin_dll_path = Path(
        f"{site_packages}/pyqt6_plugins/Qt/plugins/designer/{dll_name}")
    if not plugin_dll_path.exists():
        warnings.warn(
            f"Can't find avaliable {dll_name}, which may cause PyQt-Fluent-Widgets not being visible in QtDesigner.")
        return str(path)
